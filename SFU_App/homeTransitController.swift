...
var passNo2: NSInteger!
var stopNo2: NSString!

class customTransitCell: UITableViewCell{
    @IBOutlet weak var stopName: UILabel!
    @IBOutlet weak var timeUntil: UILabel!
    
}

class homeTransitController: UITableViewController, NSXMLParserDelegate,ENSideMenuDelegate {
    var toPass:String!
    ...
    override func viewDidLoad() {
        ...
        println(toPass)
        var url : NSURL = NSURL(string: "http://api.translink.ca/rttiapi/v1/stops/51863/estimates?apikey=jTTSvGWPEtJTFahKHoBe")!
    ...
    func parser(parser: NSXMLParser!, foundCharacters string: String!) {
        ...
        if(!data.isEmpty){
            if eName == "ExpectedLeaveTime"{
                ...
                var temp2 = data
                var temp2Arr = temp2.stringByReplacingOccurrencesOfString("/", withString: "", options: NSStringCompareOptions.LiteralSearch, range: nil)
                temp2Arr = temp2Arr.stringByReplacingOccurrencesOfString("SFU", withString: "", options: NSStringCompareOptions.LiteralSearch, range: nil)

                subCategoryTitle+=temp2Arr
            }
        }
    }
    ...
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCellWithIdentifier("transitCell", forIndexPath: indexPath) as UITableViewCell
        ...
        if (subCategoryID.isEmpty){
            (cell as customTransitCell).stopName?.text = "No buses running"
        }else{
            (cell as customTransitCell).stopName?.text = subCategorys.subCategoryTitle.capitalizedString
            (cell as customTransitCell).timeUntil?.text = minuteString //subCategorys.subCategoryID
        }
        return cell
    }
